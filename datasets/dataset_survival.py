...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self, csv_path='dataset_csv/ccrcc_clean.csv', mode='path', shuffle=False, seed=7, print_info=True,
                 n_bins=4, patient_strat=False, label_col=None, eps=1e-6):
        r"""
        ...
        # here, update the two boundaries
    ...
    def get_split_from_df_1(self, all_splits: dict, split_key: str='train', scaler=None):
        split = all_splits[split_key]
        split = split.dropna().reset_index(drop=True) # 删除可能出现的空值（有些 CSV 分组列可能结尾填充了 NaN)。

        if len(split) > 0:
            mask = self.slide_data['slide_id'].isin(split.tolist()) # 检查 self.slide_data 中的 slide_id 哪些在这次要的 split 里面，得到一个布尔掩码，比如 [True, False, True, ...]
            df_slice = self.slide_data[mask].reset_index(drop=True) # 根据这个布尔掩码，从 slide_data 中提取出当前分组对应的全部行，再重置索引，变成干净的子表
            split = Generic_Split(df_slice, metadata=self.metadata, mode=self.mode,  data_dir=self.data_dir, label_col=self.label_col, patient_dict=self.patient_dict, num_classes=self.num_classes)
        #   df_slice: 当前分组(训练或者验证)的csv数据。
        #   metadata: 元数据列名。
        #   mode, data_dir: 控制特征读取方式和数据路径, mode: 'path' 。 data_dir:'Bladder/tcga_blca_20x_features' 。
        #   label_col, patient_dict, num_classes: 保持一致性
        else:
            split = None
        
        return split

    def return_splits_1(self, from_id: bool=True, csv_path: str=None):
        if from_id:
            raise NotImplementedError
        else:
            assert csv_path 
            all_splits = pd.read_csv(csv_path)
            remove_split = self.get_split_from_df_1(all_splits=all_splits, split_key='slide_id')
        return remove_split
                ...
                if self.mode == 'path':
                    ...
                    for slide_id in slide_ids:
                        ...
                        if not os.path.exists(wsi_path):
                            wsi_bag = torch.ones(1)
                            if  len(slide_ids)>1:
                                continue
                        else:
                            wsi_bag = torch.load(wsi_path)
                            if not isinstance(wsi_bag, torch.Tensor):
                                wsi_bag = torch.tensor(wsi_bag)  # ← 添加这行
                        path_features.append(wsi_bag)

                    path_features = torch.cat(path_features, dim=0)
                    return (path_features, label, event_time, c, wsi_path)
